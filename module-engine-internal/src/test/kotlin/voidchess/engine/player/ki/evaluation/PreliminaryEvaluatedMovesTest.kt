...
internal class MinPreliminaryEvaluatedMovesTest {
    ...
    private val gameMock = mockk<EngineChessGame>()
    ...
    fun minMoveProvider(): Array<Array<Any>> {
        return arrayOf(
            arrayOf(
                listOf(Pair(Move.byCode("a1-a2"), Ongoing(-8.0, -8.5))),
                listOf(CheckmateSelf(1, -4.0)),
                CheckmateSelf(1, -4.0)
            ),
            arrayOf(
                listOf(Pair(Move.byCode("a1-a2"), Ongoing(-8.0, -8.5))),
                listOf(Ongoing(-7.0, -8.6)),
                Ongoing(-7.0, -8.6)
            ),
            arrayOf(
                listOf(Pair(Move.byCode("a1-a2"), Ongoing(-8.0, -8.5))),
                listOf(Ongoing(-7.0, -7.6)),
                Ongoing(-8.0, -8.5)
            ),
            arrayOf(
                listOf(
                    Pair(Move.byCode("a1-a2"), Ongoing(-8.0, -8.5)),
                    Pair(Move.byCode("a1-a3"), Ongoing(8.0, 8.5))
                ),
                listOf(Stalemate),
                Ongoing(-8.0, -8.5)
            ),
            ...
            arrayOf(
                listOf(
                    Pair(Move.byCode("a1-a3"), Ongoing(-8.0, -8.5)),
                    Pair(Move.byCode("a1-a2"), Ongoing(-8.2, -8.4))
                ),
                listOf(Stalemate),
                Ongoing(-8.0, -8.5)
            ),
            ...
            arrayOf(
                listOf(Pair(Move.byCode("a1-a3"), Ongoing(-8.0, -8.5))),
                listOf(
                    CheckmateSelf(3, -4.3),
                    CheckmateSelf(2, -3.3),
                    CheckmateSelf(2, -2.3)
    ...
    fun `test if minimum is found`(
        prelimEvalList: List<Pair<Move, Ongoing>>,
        selectorEvalList: List<Evaluation>,
        expectedMin: Evaluation
    ) {
        //*setup
        val forWhite = true
        ...
        for ((move, ongoing) in prelimEvalList) {
            ...
            every {
                minPreliminaryEvaluatedMovesSpy["evaluateToOngoing"](
                    any<EngineChessGame>(),
                    eq(move),
                    eq(forWhite),
                    eq(prelimEval),
                    any<EvaluatingStatically>()
                )
            } returns ongoing
        }
        ...
        assertTrue("expectedMin: $expectedMin, actualMin: $actualMin") {
...
class MaxPreliminaryEvaluatedMovesTest {
    ...
    @Test(dataProvider = "minMoveProvider")
    fun `test if maximum is found`(
        prelimEvalList: List<Pair<Move, Ongoing>>,
        selectorEvalList: List<Evaluation>,
        expectedMax: Evaluation
    ) {
        //*setup
        val forWhite = true
        ...
        for ((move, ongoing) in prelimEvalList) {
            ...
            every {
                maxPreliminaryEvaluatedMovesSpy["evaluateToOngoing"](
                    any<EngineChessGame>(),
                    eq(move),
                    eq(forWhite),
                    eq(prelimEval),
        ...
        assertTrue("expectedMax: $expectedMax, actualMin: $actualMax") {
            LowestEvaluationFirstComparator.compare(
                expectedMax,
                actualMax
            ) == 0
