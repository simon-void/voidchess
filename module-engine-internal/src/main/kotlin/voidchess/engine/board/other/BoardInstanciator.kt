...
import voidchess.engine.figures.FigureType.*
import voidchess.engine.figures.King
import voidchess.engine.figures.getPawn

internal interface BoardInstanciator {
    fun generateInitialSetup(): Iterable<Pair<Position, Figure?>>
}

internal fun StartConfig.boardInstanciator(): BoardInstanciator {
...
private fun getPawnRows(): Iterator<Pair<Position, Figure?>> = iterator<Pair<Position, Figure?>> {
    for (column in 0..7) {
        yield(Position[1, column].let { pos -> pos to getPawn(pos, true) })
        yield(Position[6, column].let { pos -> pos to getPawn(pos, false) })
    }
}
