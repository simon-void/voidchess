...
internal class CheckLineTest {
    ...
    fun testActualCheckLineIterator(kingPosCode: String, attackerPosCode: String, expectedPositionCodes: List<String>) {
        ...
        Assert.assertEquals(expectedPositionCodes.size, kingPos.distanceTo(attackerPos))
        ...
        while (posIter.hasNext()) {
            ...
            Assert.assertEquals(actualPos, expectedPositions[index], "position index: $index")
            index++
        }
        Assert.assertEquals(index, expectedPositions.size)
    }
    ...
    fun testKeepsKingInCheckIfHeMovesTo(checkLine: CheckLine, inCheckDirections: List<Direction>) {
        for(direction in Direction.entries) {
            ...
            Assert.assertEquals(
                actualInCheck,
                inCheckDirections.contains(direction),
                "no check in direction $direction"
            )
        }
    }
