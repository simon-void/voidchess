...
class AttackLinesTest {
    ...
    fun testCheckAttackLines(gameDes: String, isWhite: Boolean, attackerGivingCheckPosCodes: Set<String>, boundFigurePosCodes: Set<String>) {
        ...
        Assert.assertEquals(attackLines.noCheck, actualChecks == 0, "noCheck")
        Assert.assertEquals(attackLines.isSingleCheck, actualChecks == 1, "isSingleCheck")
        Assert.assertEquals(attackLines.isDoubleCheck, actualChecks == 2, "isDoubleCheck")
        Assert.assertEquals(attackLines.isCheck, actualChecks != 0, "isCheck")

        // test checkLines
        Assert.assertEquals(attackLines.checkLines.size, attackerGivingCheckPosCodes.size, "number of checks")
        Assert.assertEquals(
        ...
        // test boundLines
        Assert.assertEquals(
            attackLines.boundLineByBoundFigurePos.size,
            boundFigurePosCodes.size,
            "number of bound lines"
        )
        Assert.assertEquals(
    ...
    fun testCheckAttackLinesIteratedOrder() {
        ...
        Assert.assertEquals(attackLines.checkLines.size, 1, "#checkLines")
        Assert.assertEquals(attackLines.boundLineByBoundFigurePos.size, 1, "#boundLines")

        fun assertIteratorOrder(positionProgression: PositionProgression, vararg posCodes: String) {
            Assert.assertEquals(positionProgression.toList(), posCodes.map { Position.byCode(it) }, "ordered")
        }
